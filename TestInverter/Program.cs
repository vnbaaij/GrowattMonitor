...
namespace GrowattMonitorShared
{
    ...
    public class Program
    {
        ...
        private static readonly byte[] IDENTIFY_11 = new byte[] {
                0x00, 0x01, 0x00, 0x05, 0x00, 0x1C, 0x01, 0x19, 0x0D, 0x22, 0x2C, 0x43, 0x58, 0x46, 0x46, 0x75,
                0x45, 0x2E, 0x77, 0x70, 0x74, 0x78, 0x76, 0x4B, 0x5D, 0x59, 0x50, 0x42, 0x4C, 0x69, 0x43, 0x41,
                0x4E, 0x58, 0xA7, 0xDC
            };
        ...
        private static Message ProcessMessage(Message msg)
        {
            ...
            switch (msg.Type)
            {
                ...
                case MessageType.ANNOUNCE:
                    ...
                    {
                        Console.WriteLine("==> Announce request received");

                        SendAnnounce();
                    }
                    break;
                case MessageType.IDENTIFY:
                    if (_state == InverterState.PING_SENT )
                    {
                        _state = InverterState.IDENTIFY_RCVD;
                        var idreqdata = msg.DecodeIdentifyRequest();
                        Console.WriteLine($"==> Identify Received; Acknowledging request for config items {idreqdata["first"]} to {idreqdata["last"]}...");
        ...
        private static void SendConfig()
        {
            foreach (var buffer in IDENTIFY)
            {
                ...
                SendMessage(msg);
                //{cfg.Display()}
                Console.WriteLine($"==> Sent Config: {cfg.Display()}"); // item details (0x{BitConverter.ToUInt16(msg.Body[10..12].ReverseWhenLittleEndian()):X2}");
                Thread.Sleep(500);
            }
            _state = InverterState.IDENTIFY_SENT;
        }
