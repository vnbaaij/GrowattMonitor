...
public class Telegram : ITableEntity
{
    ...
    public Telegram(byte[] buffer, ILoggerFactory? factory) : this(factory)
    {
        ...
        InverterTimestamp = new DateTime(2000+buffer[28], buffer[29], buffer[30], buffer[31], buffer[32], buffer[33], DateTimeKind.Utc);
        ...
        energy = buffer[39..^0];
        Type classType= typeof(Telegram);

        foreach (var d in GetDataList())
        {
            object value = d.GetFromBuffer(energy!);
